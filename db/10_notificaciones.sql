...
create policy "sp_notifc_write" on notif_config
  for all to authenticated using (usuario_id = auth.uid() or public.is_admin())
...
-- Solo admins/gerencia pueden ver la cola
...
-- Cualquiera puede insertar (los triggers lo hacen via service_role de hecho)
...
-- Crear config por default al crear perfil
create or replace function sumipanel.handle_new_user()
returns trigger
language plpgsql
security definer
set search_path = public, sumipanel
as $$
begin
  ...
  -- Crear config de notificaciones con defaults
  insert into sumipanel.notif_config (usuario_id)
  values (new.id)
...
-- =============================================================
create or replace function sumipanel.enqueue_email(
  p_to text,
  p_to_name text,
  p_subject text,
  p_html text,
...
as $$
begin
  insert into sumipanel.notif_queue (to_email, to_name, subject, html_body, tipo, ref_codigo, metadata)
...
-- TEMPLATES HTML
-- =============================================================
create or replace function sumipanel.template_recepcion(p_folio text, p_oc text, p_proveedor text, p_conformidad text, p_user_name text)
returns text
language sql
immutable
as $$
  select '<div style="font-family:-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif;max-width:600px;margin:0 auto;background:#0b0d12;color:#e6e9f2;padding:0">
...
create or replace function sumipanel.template_stock_critico(p_productos text, p_user_name text)
returns text
language sql
immutable
as $$
...
create or replace function sumipanel.template_tarea(p_titulo text, p_user_name text, p_fecha_limite text, p_tipo text)
returns text
language sql
immutable
as $$
...
create or replace function sumipanel.template_oc_atrasada(p_oc text, p_proveedor text, p_dias int, p_user_name text)
returns text
language sql
immutable
as $$
...
-- =============================================================
-- TRIGGERS: detectar eventos y encolar mails
-- =============================================================

-- 1) Recepcion registrada -> mail a admin y a solicitante de la OC
create or replace function sumipanel.trg_recepcion_mail()
returns trigger
language plpgsql
security definer
set search_path = public, sumipanel
...
begin
  ...
  v_html := sumipanel.template_recepcion(new.folio, v_oc_folio, v_prov, new.conformidad, 'equipo');

  -- Mail a admins y gerentes
  for v_user in
  ...
  loop
    ...
    perform sumipanel.enqueue_email(
      v_user.email, v_user_name,
      'Recepción ' || new.folio || ' - OC ' || v_oc_folio,
      sumipanel.template_recepcion(new.folio, v_oc_folio, v_prov, new.conformidad, v_user_name),
      'recepcion',
      new.folio
    );
  end loop;
...
create trigger tg_recep_mail
  ...
  for each row execute function sumipanel.trg_recepcion_mail();

-- 2) Tarea asignada -> mail al asignado
create or replace function sumipanel.trg_tarea_mail()
returns trigger
language plpgsql
security definer
set search_path = public, sumipanel
  ...
  if new.asignado_id is not null and new.asignado_id != new.creador_id then
    ...
    if v_user_email is not null then
      ...
      perform sumipanel.enqueue_email(
        ...
        sumipanel.template_tarea(new.titulo, v_user_name, v_fecha, 'asignada'),
        'tarea',
        new.id::text
      );
    end if;
...
create trigger tg_tarea_mail
  ...
  for each row execute function sumipanel.trg_tarea_mail();

-- 3) Stock bajo minimo -> mail a admin/compras
create or replace function sumipanel.trg_stock_mail()
returns trigger
language plpgsql
security definer
set search_path = public, sumipanel
...
declare
  ...
  v_count int;
begin
  if (new.stock <= new.stock_min) and (old.stock > old.stock_min or old.stock is null) then
    -- Listar todos los productos criticos
    ...
    if v_count > 0 then
      ...
      loop
        ...
        perform sumipanel.enqueue_email(
          ...
          sumipanel.template_stock_critico(v_productos, v_user_name),
          'stock',
          null
        );
      end loop;
...
create trigger tg_stock_mail
  ...
  for each row execute function sumipanel.trg_stock_mail();

-- =============================================================
-- Funcion para que la Edge Function consulte la cola
-- =============================================================
-- La Edge Function va a hacer SELECT FROM notif_queue WHERE estado='pendiente'
-- y los marca como enviados.

select 'Schema de notificaciones listo. Siguiente: desplegar Edge Function.' as status;
